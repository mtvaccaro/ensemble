...
export function TranscriptionButton({
  ...
  const { status, transcript, error, isLoading, startTranscription, checkStatus } = useTranscription(episodeId)
  ...
  useEffect(() => {
    ...
    if (status === TranscriptionStatus.COMPLETED && transcript && onTranscriptionComplete) {
      onTranscriptionComplete(transcript)
    }
  }, [status, transcript, onTranscriptionComplete])
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {hasTranscript && currentStatus === TranscriptionStatus.COMPLETED && (
        <details className="text-sm border border-gray-200 rounded-md p-2">
          <summary className="cursor-pointer font-medium hover:text-blue-600">
            View Transcript
          </summary>
          <div className="mt-2 text-gray-700 whitespace-pre-wrap max-h-96 overflow-y-auto">
